...
// Zoom into 20x20 meter area
const constexpr auto kMaxLaserDistance = 40 * 100.;
...
int main(int argc, char* argv[]) try {
  ...
  while (window.isOpen()) {
    ...
    for (auto sample : scan.samples) {
      ...
      x = (x / (2 * kMaxLaserDistance)) * windowSize.x;
      y = (y / (2 * kMaxLaserDistance)) * windowSize.y;
